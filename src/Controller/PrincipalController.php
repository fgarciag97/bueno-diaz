...
class PrincipalController extends AppController
{
    public function beforeFilter(Event $event){
        ...
        $this->Auth->allow(['index', 'login', 'menu', 'registro', 'contacto', 'informacion', 'carrito', 'agregarCarrito', 'tramites', 'detalle', 'search', 'recuperar', 'buscador', 'verificacion']);
    ...
    public function login(){
        if($this->request->is('post')){
            ...
            if($user){
                ...
                /*-----------------------------------------------------------------------------------------------*/
                if(!$user['autenticado']){
                    $this->Flash->error('User not validated. Check your email');
                    ...
                    @$correo
                      ->setTransport('password') //nombre del configTrasnport que acabamos de configurar
                      ->setEmailFormat('html') //formato de correo
                      ->setTo(@$usuario['correo']) //correo para
                      ->setSubject('Verification email') //asunto
                      ->setFrom(['admin@buenodiaz.com' => 'Bueno&Diaz']) //correo de
                      ->ViewVars([ //enviar variables a la plantilla
                        'codigo' => @$usuario['cod_autenticado']
                      ]);

                    $correo->viewBuilder()->setTemplate('verificacion');
                    $correo->send();
    ...
    public function registro(){
        ...
        /*---------------------------------------------------------------------------------------------*/
        $registro->autenticado = false;
        $registro->cod_autenticado = $this->cod_autenticado();
        /*---------------------------------------------------------------------------------------------*/
            ...
            if($n == 0){
                if ($n2 == 0) {
                    if ($this->Usuarios->save($registro)) {
                        ...
                        /*---------------------------------------------------------------------------------------------*/
                        $this->Flash->success(__('An email has been sent for your verification'));

                        $correo = new Email(); //instancia de correo
                        @$correo
                          ->setTransport('password') //nombre del configTrasnport que acabamos de configurar
                          ->setEmailFormat('html') //formato de correo
                          ->setTo($registro->correo) //correo para
                          ->setSubject('Verification email') //asunto
                          ->setFrom(['admin@buenodiaz.com' => 'Bueno&Diaz']) //correo de
                          ->ViewVars([ //enviar variables a la plantilla
                            'codigo' => @$registro->cod_autenticado
                          ]);
    ...
    public function verificacion($codigo = null){
        $this->autoRender = false;

        if($codigo != null){
            $this->loadModel('Usuarios');
            $n = @$this->Usuarios->find('all')->where(['cod_autenticado' => $codigo])->count();
            
            if($n != 0){
                $this->Flash->success(__('Verified user. Please log in'));

                $this->loadModel('Usuarios');
                $variable = $this->Usuarios->query();
                $variable->update()
                    ->set(['autenticado' => true])
                    ->where(['cod_autenticado' => $codigo])
                    ->execute(); 

                return $this->redirect(['action' => 'index']);
            }else{
                $this->Flash->error(__('Invalid validation code'));
                return $this->redirect(['action' => 'index']);
            }
        }else{
            $this->Flash->error(__('Invalid validation code'));
    ...
    public function reporte($id_carrito = null){
     ...
     public function recuperar(){
        ...
        if($usuario['correo'] != null){ 
            ...
            if(!$correo->send()){
              $this->Flash->info(__('Alert email has not been sent. Mail delivery for TODAY has collapsed'));
            }else{
                $this->Flash->success('Password sent to your email');
            }
